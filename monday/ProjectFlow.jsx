...
/* ═══════════════════════════════════════════════════════════
   CONFIGURAÇÃO — edite API_BASE para o URL do seu Worker
═══════════════════════════════════════════════════════════ */
const API_BASE = (typeof window !== "undefined" && window.__PF_API__)
  || import.meta?.env?.VITE_API_URL
  || "https://projectflow-api.SEU_SUBDOMINIO.workers.dev";
...
function Login({onLogin}) {
  ...
  const submit = async () => {
    if(!form.email||!form.password) return setErr("Preencha todos os campos");
    ...
    <div className="login-wrap">
      <div className="login-box">
        <div style={{textAlign:"center",marginBottom:24}}>
          ...
          <h1 style={{fontSize:20,fontWeight:700,color:"#0f172a"}}>ProjectFlow</h1>
          <p style={{fontSize:12,color:"#64748b",marginTop:4}}>Gestão de projetos</p>
        </div>
        {err && <div style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:6,padding:"8px 12px",marginBottom:12,fontSize:12,color:"#991b1b"}}>{err}</div>}
        <Field label="Email"><input type="email" value={form.email} onChange={e=>set("email",e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="seu@email.com"/></Field>
        <Field label="Senha"><input type="password" value={form.password} onChange={e=>set("password",e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="••••••••"/></Field>
        <button className="btn bp" style={{width:"100%",justifyContent:"center",padding:"10px",marginTop:4}} onClick={submit} disabled={loading}>
  ...
  return (
    <div>
      <div className="frb mb4">
        ...
        <button className="btn bp bsm" onClick={()=>nav("projects")}>+ Novo Projeto</button>
      </div>
      ...
      <div className="g2 mb4">
        <div className="card">
          ...
          <div className="fc fg3">
            ...
            {projs.map(p=>{
              ...
              return (
                <div key={p.id} onClick={()=>nav("kanban",p.id)} style={{cursor:"pointer"}}>
        ...
        <div className="card">
          <div className="frb mb3">
            ...
            <button className="btn bxs bg" onClick={()=>nav("milestones")}>Ver todos →</button>
          </div>
          {lm ? <Skeleton/> :
      ...
      <div className="card">
        <div className="frb mb3">
          ...
          <button className="btn bxs bg" onClick={()=>nav("tasks")}>Ver todas →</button>
        </div>
        {lt ? <Skeleton/> :
    ...
    <div>
      ...
      <div className="g3">
        {projs.map(p=>{
          ...
          const pct   = total?Math.round(done/total*100):0;
          return (
            <div key={p.id} className="card" style={{borderTop:`3px solid ${p.color||"#3b82f6"}`,cursor:"pointer"}} onClick={()=>nav("kanban",p.id)}>
...
const NAV_ITEMS = [
  ...
  {page:"tracker",   icon:"⏱",  label:"Time Tracker"},
  {page:"wiki",      icon:"📖", label:"Wiki"},
  {page:"milestones",icon:"◆",  label:"Milestones"},
  {page:"autos",     icon:"⚡", label:"Automações"},
  {page:"reports",   icon:"📈", label:"Relatórios"},
  {page:"settings",  icon:"⚙",  label:"Config."},
];

const PAGE_TITLES = {
  dash:"Dashboard",projects:"Projetos",kanban:"Kanban",tasks:"Tarefas",
  timeline:"Timeline",tracker:"Time Tracker",wiki:"Wiki",milestones:"Milestones",
  autos:"Automações",reports:"Relatórios",settings:"Configurações"
};
...
function AppShell() {
  ...
  const nav = useCallback((p, projectId) => {
    setPage(p);
  ...
  useEffect(()=>{
    const h = e => {
      ...
      if(e.key==="k"||e.key==="K") nav("kanban");
      if(e.key==="t"||e.key==="T") nav("timeline");
      if(e.key==="r"||e.key==="R") nav("reports");
      if(e.key==="n"||e.key==="N") nav("tasks");
      if(e.key==="p"||e.key==="P") nav("projects");
    };
  ...
  const renderPage = () => {
    switch(page){
      case "dash":       return <Dashboard nav={nav}/>;
      case "projects":   return <Projects  nav={nav}/>;
      case "kanban":     return <Kanban    pid={pid} nav={nav}/>;
      case "tasks":      return <TaskList/>;
      case "timeline":   return <Timeline/>;
      case "tracker":    return <Tracker/>;
      case "wiki":       return <Wiki/>;
      case "milestones": return <Milestones/>;
      case "autos":      return <Automations/>;
  ...
  return (
    <div className="app">
      <div className={`sb ${sideOpen?"open":""}`}>
        <div className="sb-logo"><span>⚡</span> ProjectFlow</div>
        <div className="sb-sec">
          {NAV_ITEMS.map(({page:p,icon,label})=>(
            <div key={p} className={`nav ${page===p?"on":""}`} onClick={()=>nav(p)}>
              <span className="nav-ic">{icon}</span>{label}
            </div>
          ))}
        </div>
        <div style={{padding:"10px 14px",borderTop:"1px solid rgba(255,255,255,.08)"}}>
          <div className="fr fg2 mb2">
            <Av name={user?.name||"U"} color="#3b82f6"/>
            <div style={{minWidth:0}}>
              <div className="tsm fw6" style={{color:"#e2e8f0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:120}}>{user?.name}</div>
              <div className="txs" style={{color:"#475569"}}>{user?.role}</div>
            </div>
          </div>
          <button className="btn bg bsm" style={{width:"100%",justifyContent:"center",color:"#94a3b8",borderColor:"rgba(255,255,255,.1)"}} onClick={logout}>Sair</button>
        </div>
      </div>

      <div className={`mob-ov ${sideOpen?"open":""}`} onClick={()=>setSideOpen(false)}/>

      <div className="main">
        <div className="topbar">
          <div className="fr fg2">
            <button className="ham" onClick={()=>setSideOpen(s=>!s)}>☰</button>
            <span className="tb-title">{PAGE_TITLES[page]||""}</span>
          </div>
          <div className="fr fg2">
            <span className="txs tm hide-m">{API_BASE.replace("https://","").split(".")[0]}</span>
            <Av name={user?.name||"U"} color="#3b82f6"/>
          </div>
        </div>
...
function AuthProvider({children}) {
  ...
  const login = useCallback((t, u) => {
    sessionStorage.setItem("pf_token", t);
