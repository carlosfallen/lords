...
import { Search, Bell, Zap, LogOut, User, Menu } from 'lucide-react';
...
export default function Header({ onMenuClick }: HeaderProps) {
    ...
    return (
        <header className="h-14 border-b border-dark-800 bg-dark-900/50 backdrop-blur-sm flex items-center justify-between px-4 md:px-6 shrink-0">
            <div className="flex items-center gap-2 md:gap-4">
                <button
                    onClick={onMenuClick}
                    className="p-2 -ml-2 hover:bg-dark-800 rounded-lg md:hidden transition-colors text-dark-400"
                >
                    <Menu size={20} />
                </button>
                <h2 className="text-sm md:text-base font-semibold text-dark-100 truncate">{title}</h2>
