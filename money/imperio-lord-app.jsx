...
// ─── CONFIGURAÇÃO DA API ──────────────────────────────────────────────────────
// ⚠️  Substitua pela URL do seu Cloudflare Worker após deploy
const API = "https://SEU_WORKER.workers.dev";
...
function useAPI(endpoint, deps=[]) {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${API}${ref.current}`);
...
const ErrBox = ({ msg, onRetry }) => (
  <div style={{
    ...
    <div style={{ color:C.mid, fontSize:11, marginBottom:8 }}>
      Verifique se a URL do Worker está configurada em <code style={{ color:C.gold }}>const API</code> no topo do arquivo.
    </div>
...
function AddModal({ onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    setSave(true); setErr("");
    try {
      const r = await fetch(`${API}/api/movimentacoes`, {
        method:"POST",
    ...
    <div style={{
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{ padding:"12px 20px 0" }}>
          <div style={{ fontWeight:800, fontSize:17, marginBottom:16 }}>Nova Movimentação</div>
  ...
  return (
    <div style={{ padding: desktop ? "28px 32px" : "16px 16px 0" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24 }}>
        <div>
          <div style={{ color:C.mid, fontSize:11, letterSpacing:1.5, textTransform:"uppercase" }}>Império Lord</div>
...
function Caixa({ desktop, refreshKey }) {
  ...
  return (
    <div style={{ padding: desktop ? "28px 32px" : "16px 16px 0" }}>
      <div style={{ fontSize: desktop ? 24 : 18, fontWeight:800, marginBottom:8 }}>Caixa & Movimentações</div>
...
function Recebiveis({ desktop }) {
  ...
  const marcar = async (id, status) => {
    setSav(id);
    try {
      await fetch(`${API}/api/parcelas/${id}`, {
        method:"PATCH",
        headers:{"Content-Type":"application/json"},
  ...
  return (
    <div style={{ padding: desktop ? "28px 32px" : "16px 16px 0" }}>
      <div style={{ fontSize: desktop ? 24 : 18, fontWeight:800, marginBottom:18 }}>Recebíveis</div>
          ...
          {lista.map(r=>(
            <Card key={r.id} style={{
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                <div>
                  <div style={{ fontWeight:700, fontSize:14 }}>{r.cliente_nome || "—"}</div>
...
function Missoes({ desktop }) {
  ...
  const toggle = async (m) => {
    setSav(m.id);
    try {
      await fetch(`${API}/api/missoes/${m.id}`, {
        method:"PATCH",
        headers:{"Content-Type":"application/json"},
  ...
  return (
    <div style={{ padding: desktop ? "28px 32px" : "16px 16px 0" }}>
      <div style={{ fontSize: desktop ? 24 : 18, fontWeight:800, marginBottom:4 }}>Missões do Dia</div>
...
function Pipeline({ desktop }) {
  ...
  return (
    <div style={{ padding: desktop ? "28px 32px" : "16px 16px 0" }}>
      <div style={{ fontSize: desktop ? 24 : 18, fontWeight:800, marginBottom:4 }}>Pipeline de Vendas</div>
      <div style={{ color:C.mid, fontSize:13, marginBottom:20 }}>
      ...
      {lista.length === 0 ? (
        ...
        estagios.map(est=>{
          const items = lista.filter(p=>p.estagio===est);
          if (items.length === 0) return null;
          const sub = items.reduce((s,p)=>s+p.valor,0);
          return (
            <div key={est} style={{ marginBottom:20 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <div style={{ width:10, height:10, borderRadius:99, background:EST_COR[est] }} />
                  <span style={{ fontWeight:700, fontSize:15 }}>{EST_LBL[est]}</span>
                  <span style={{ color:C.mid, fontSize:12 }}>({items.length})</span>
                </div>
                <span style={{ color:EST_COR[est], fontWeight:700 }}>{R$(sub)}</span>
              </div>
              <div style={{
                display: desktop ? "grid" : "flex",
                gridTemplateColumns: desktop ? "repeat(auto-fill,minmax(280px,1fr))" : undefined,
                ...
                {items.map(p=>(
                  <Card key={p.id} style={{ padding:"12px 14px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      <div>
                        <div style={{ fontWeight:700, fontSize:14 }}>{p.cliente}</div>
                        <div style={{ color:C.mid, fontSize:12 }}>{p.servico}</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ color:C.gold, fontWeight:800, fontSize:16 }}>{R$(p.valor)}</div>
                        {est !== "fechamento" && (
                          <button
                    ...
                    {p.observacao && (
                      <div style={{ color:C.mid, fontSize:11, marginTop:8 }}>{p.observacao}</div>
                    )}
                  </Card>
                ))}
              </div>
...
const TABS = [
  ...
  { id:"recebiveis", icon:"📋", label:"Receber"   },
        ...
        <button key={t.id} onClick={()=>onChange(t.id)} style={{
          ...
          {active===t.id && (
            <div style={{ position:"absolute", top:0, left:"50%", transform:"translateX(-50%)",
              width:28, height:2, background:C.gold, borderRadius:99 }} />
          )}
          <div style={{ fontSize:19 }}>{t.icon}</div>
          <div style={{ fontSize:10, fontWeight:600, color: active===t.id ? C.gold : C.dim }}>{t.label}</div>
        </button>
      ))}
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ padding:"0 20px 28px" }}>
        <div style={{ color:C.mid, fontSize:10, letterSpacing:2, marginBottom:4 }}>IMPÉRIO LORD</div>
        <div style={{ color:C.gold, fontWeight:900, fontSize:18 }}>Painel</div>
      </div>
      {TABS.map(t=>(
        <button key={t.id} onClick={()=>onChange(t.id)} style={{
          background: active===t.id ? `${C.gold}12` : "transparent",
          border: "none",
          borderLeft: `3px solid ${active===t.id ? C.gold : "transparent"}`,
          padding:"13px 20px", cursor:"pointer",
          display:"flex", alignItems:"center", gap:12,
          color: active===t.id ? C.gold : C.mid,
      ...
      <div style={{ flex:1 }} />
      <div style={{ padding:"0 20px", color:C.dim, fontSize:11 }}>
        <div style={{ marginBottom:4 }}>🟢 Cloudflare D1</div>
        <div style={{ wordBreak:"break-all", fontSize:10 }}>{API}</div>
      </div>
    </div>
  );
}
...
// ─── FAB ──────────────────────────────────────────────────────────────────────
function FAB({ onClick, desktop }) {
  return (
    <button onClick={onClick} style={{
      position:"fixed",
      bottom: desktop ? 32 : 72,
      right:  desktop ? 32 : 20,
      width:54, height:54, borderRadius:99,
      background:C.gold, border:"none", fontSize:26,
      cursor:"pointer", color:"#000", fontWeight:900,
      boxShadow:`0 6px 24px ${C.gold}55`,
      display:"flex", alignItems:"center", justifyContent:"center",
      zIndex:300, transition:"transform 0.1s, box-shadow 0.1s",
      fontFamily:"inherit"
    }}
    onMouseDown={e=>{ e.currentTarget.style.transform="scale(0.91)"; e.currentTarget.style.boxShadow=`0 2px 10px ${C.gold}55`; }}
    onMouseUp={e=>  { e.currentTarget.style.transform="scale(1)";    e.currentTarget.style.boxShadow=`0 6px 24px ${C.gold}55`; }}
    >+</button>
  );
}

// ─── APP PRINCIPAL ────────────────────────────────────────────────────────────
export default function ImperioLord() {
  ...
  const screenProps = { desktop, refreshKey: refresh };

  const content = (
    <div style={{ flex:1, overflowY:"auto" }}>
      {tab === "home"       && <Dashboard    {...screenProps} />}
      {tab === "caixa"      && <Caixa        {...screenProps} />}
      {tab === "recebiveis" && <Recebiveis   {...screenProps} />}
      {tab === "pipeline"   && <Pipeline     {...screenProps} />}
      {tab === "missoes"    && <Missoes      {...screenProps} />}
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <FAB onClick={() => setModal(true)} desktop={desktop} />
